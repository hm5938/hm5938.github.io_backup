---
layout: post
title:  "[알고리즘]   크레인 인형뽑기게임"
subtitle: "test"
categories: algorithm
tags: algorithm programmers 64061
comments: false
---
### **문제 설명**
...
[제목 없음](https://www.notion.so/b08e5002f4e2470196f658a3b79c50e3)
...
1. moves 배열만큼 반복한다.
2. 해당하는 라인의 인형을 첫번째줄 부터 스캔한다. 인형이 있는 줄을 찾는다.
3. 만약 끝줄까지 스캔했을 때에도 인형이 없으면 다음 차례로 넘어간다.
4. 인형이있는 줄의 인형과 그전 차례에 뽑은 인형을 비교후 같으면 그전 차례에 뽑은 인형을 지워준다.
5. 같지 않다면 뽑은 인형리스트에 인형을 추가한다.
6. 원본 배열에서 해당 인덱스의 인형을 0으로 바꾼다. 
...
import java.util.ArrayList;
 class Solution {
  public static int solution(int[][] board, int[] moves) {
          ...
          b:  for(int move : moves){
                int i=0;
                while(board[i][move-1]==0){
                    if(i==board.length-1)continue b;
                    i++;
                }
                
                if(list.size()!=0&&list.get(list.size()-1)==board[i][move-1]) {
                    list.remove(list.size()-1);
                    answer+=2;
                }
                else list.add(board[i][move-1]);
                board[i][move-1] =0;
